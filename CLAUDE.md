...
- **Fixed 2026-08-20:** `_normalize_posted_at`'s ISO-8601 check (previous gotcha) accepts offset-less strings like `"2026-08-19T10:00:00"` as valid — `fromisoformat()` parses them fine, it just returns a naive `datetime`. `clean_post()`'s vendor-forum staleness check then compared that naive value directly against a tz-aware `datetime.now(utc)`, raising `TypeError: can't compare offset-naive and offset-aware datetimes` and crashing the entire `discover` run. Fixed by assuming UTC (`posted_dt.replace(tzinfo=dt.timezone.utc)`) when `posted_dt.tzinfo is None`, matching how the `"Z"` suffix is already handled.
...
- **`get_archive_pain_posts()`'s `status_changed_at` fix (see `schema/pain_miner.sql`, fixed 2026-08-27) is live in production** — applied by hand to the `vzyknkrcmerorwfguscx` (LAI NorkForce-HubRM) Supabase project and verified via direct RPC call: of the 500 rows in the archive view, 499 have `actioned_at IS NULL` (auto-archived, never touched by a human) and — post-fix — all 499 correctly appear under a time-filtered query (they were invisible pre-fix); a tight recent cutoff and a future cutoff both correctly filter the count, confirming the RPC is discriminating on `status_changed_at`, not just returning everything.
